...
module DockerHub
  class RepoCreationRequest
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DockerHub::RepoCreationRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @name !~ Regexp.new(/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/)
    ...
    def full_description=(full_description)
      if full_description.nil?
        fail ArgumentError, 'full_description cannot be nil'
      end
